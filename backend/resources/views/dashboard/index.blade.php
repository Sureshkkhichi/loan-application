@extends('layouts.app', ['title' => 'Dashboard - LoanDesk Operations'])
...
<div class="space-y-6">
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 bg-white p-6 rounded-2xl border border-borderline shadow-sm">
        <div>
            ...
            <p class="text-sm text-gray-500 mt-0.5">Manage incoming loan leads, sales reviews, and external banker handoffs.</p>
        </div>
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-3">
        <a href="{{ route('dashboard') }}" class="p-4 rounded-xl border {{ !request('status') ? 'border-brand bg-brand/5 ring-1 ring-brand' : 'border-borderline bg-white' }} shadow-sm hover:shadow transition-all">
        ...
        <a href="{{ route('dashboard', ['status' => 'NEW']) }}" class="p-4 rounded-xl border {{ request('status') === 'NEW' ? 'border-blue-500 bg-blue-50 ring-1 ring-blue-500' : 'border-borderline bg-white' }} shadow-sm hover:shadow transition-all">
        ...
        <a href="{{ route('dashboard', ['status' => 'IN_PROGRESS']) }}" class="p-4 rounded-xl border {{ request('status') === 'IN_PROGRESS' ? 'border-indigo-500 bg-indigo-50 ring-1 ring-indigo-500' : 'border-borderline bg-white' }} shadow-sm hover:shadow transition-all">
        ...
        <a href="{{ route('dashboard', ['status' => 'SUBMITTED_FOR_REVIEW']) }}" class="p-4 rounded-xl border {{ request('status') === 'SUBMITTED_FOR_REVIEW' ? 'border-amber-500 bg-amber-50 ring-1 ring-amber-500' : 'border-borderline bg-white' }} shadow-sm hover:shadow transition-all">
        ...
        <a href="{{ route('dashboard', ['status' => 'READY_FOR_BANK']) }}" class="p-4 rounded-xl border {{ request('status') === 'READY_FOR_BANK' ? 'border-teal-500 bg-teal-50 ring-1 ring-teal-500' : 'border-borderline bg-white' }} shadow-sm hover:shadow transition-all">
        ...
        <a href="{{ route('dashboard', ['status' => 'PENDENCY_RAISED']) }}" class="p-4 rounded-xl border {{ request('status') === 'PENDENCY_RAISED' ? 'border-rose-500 bg-rose-50 ring-1 ring-rose-500' : 'border-borderline bg-white' }} shadow-sm hover:shadow transition-all">
        ...
        <a href="{{ route('dashboard', ['status' => 'COMPLETED']) }}" class="p-4 rounded-xl border {{ request('status') === 'COMPLETED' ? 'border-emerald-500 bg-emerald-50 ring-1 ring-emerald-500' : 'border-borderline bg-white' }} shadow-sm hover:shadow transition-all">
        ...
        <a href="{{ route('dashboard', ['status' => 'REJECTED']) }}" class="p-4 rounded-xl border {{ request('status') === 'REJECTED' ? 'border-gray-500 bg-gray-50 ring-1 ring-gray-500' : 'border-borderline bg-white' }} shadow-sm hover:shadow transition-all">
    ...
    <div class="bg-white p-4 rounded-2xl border border-borderline shadow-sm flex flex-col md:flex-row items-center justify-between gap-4">
        <form method="GET" action="{{ route('dashboard') }}" class="flex-1 flex flex-col sm:flex-row gap-3 w-full">
            @if(request('status'))
            ...
            <div class="relative flex-1">
                <input type="text" name="search" value="{{ request('search') }}"
                    ...
                    class="w-full pl-10 pr-4 py-2.5 rounded-xl border border-borderline focus:outline-none focus:ring-2 focus:ring-brand/30 text-sm bg-bglight">
            ...
            <button type="submit" class="px-5 py-2.5 bg-brand hover:bg-brand-dark text-white rounded-xl text-sm font-semibold transition-all">
                Filter Leads
            </button>
            @if(request('search') || request('status'))
                <a href="{{ route('dashboard') }}" class="px-4 py-2.5 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-xl text-sm font-medium transition-all text-center">
                    Reset
                </a>
            @endif
        </form>
        ...
        <div class="flex items-center space-x-2">
            <a href="{{ route('dashboard', array_merge(request()->query(), ['scope' => request('scope') === 'my' ? null : 'my'])) }}"
                class="px-3.5 py-2 text-xs font-semibold rounded-xl border transition-all {{ request('scope') === 'my' ? 'bg-brand text-white border-brand' : 'bg-gray-50 text-gray-700 border-borderline' }}">
                {{ request('scope') === 'my' ? '✓ Showing My Leads' : 'Show Only My Leads' }}
            </a>
        </div>
        @endif
    </div>
